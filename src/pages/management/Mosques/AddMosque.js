...
export default function AddMosque() {
  ...
  // جلب الفروع وأنواع المساجد وأرقام المساجد الموجودة عند تحميل الصفحة
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (mosquesResponse.ok) {
          ...
          setExistingMosqueNumbers(mosquesData.map((mosque) => mosque.number));
        } else {
  ...
  // للتحقق من هيكل البيانات المستلمة
  useEffect(() => {
    if (branches.length > 0) {
      console.log("Current branches:", branches);
    }
    if (mosqueTypes.length > 0) {
      console.log("Current mosque types:", mosqueTypes);
    }
  }, [branches, mosqueTypes]);
  ...
  const validateForm = () => {
    ...
    if (!formData.number) {
      ...
      isValid = false;
    } else if (existingMosqueNumbers.includes(formData.number)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data && data.message) {
          if (typeof data.message === "object") {
            ...
            setErrors({ ...errors, general: "فشلت عملية الإضافة." });
          }
        } else {
          setErrors({ ...errors, general: "فشلت عملية الإضافة." });
        }
      ...
      setErrors({ ...errors, general: "حدث خطأ." });
      console.error(err);
    }
  };
  return (
      ...
      <div className="homepage">
        ...
        <div className="subhomepage">
          ...
          <form className="divforconten" onSubmit={handleSubmit}>
            <Managementdata dataname="بيانات المسجد" />
